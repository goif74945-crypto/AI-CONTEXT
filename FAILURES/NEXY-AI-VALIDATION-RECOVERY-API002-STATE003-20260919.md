...
FAILURE_ID: NEXY-CI-DISCOVERY-001
context: Vitest 4.1.6 scripts used --dir with include patterns rooted at tests/**.
cause: scan-root/filter mismatch produced No test files found.
failed_approach: vitest run --dir tests/contract and vitest run --dir tests/integration.
recovery: positional path filters with verbose reporter.
boundary: do not use passWithNoTests or weaken include rules.
prevention: CI must execute >0 files/tests.

FAILURE_ID: NEXY-RELEASE-BLOCKERS-20260919
context: validation run 35448899862.
cause: pre-existing typecheck diagnostics plus evidence seal mismatch/failure.
failed_approach: not remediated in this authorized scope.
recovery: separate scoped remediation.
boundary: do not conflate targeted finding PASS with project release readiness.
prevention: maintain separate targeted and release-gate verdicts.
