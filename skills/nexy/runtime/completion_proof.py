...
def env(sid, action="ANALYZE"):
    data={"GOV-001":{"sources":[{"source_id":"CMD-0001","source_type":"CURRENT_USER_COMMAND","rank":1,"status":"VERIFIED"}]},
          "GOV-002":{"request":{"target":"skills/nexy"}},
          "CTX-001":{},
...
def main():
    ...
    for sid,e in rt.registry["skills"].items():
        assert callable_ref(e["runtime_entry"])
        assert callable_ref(e["validator"])
        assert callable_ref(e["evidence_handler"])
    success=rt.run("GOV-001",env("GOV-001"))
    assert success["status"]=="SUCCESS"
    assert success["chain"]==["LOAD","RESOLVE","AUTHORIZE","EXECUTE","VALIDATE","EVIDENCE","RESULT"]
    failure=rt.run("GOV-001",env("GOV-001",action="WRITE"))
    assert failure["status"]=="FAILURE" and failure["failure"]["code"]=="UNAUTHORIZED"
    freeze=env("CTX-003")
    freeze["data"]["files"][0]["content"]="tampered"
    freeze["data"]["files"][0]["content_hash"]="sha256:"+"0"*64
    frozen=rt.run("CTX-003",freeze)
    assert frozen["status"]=="FREEZE" and frozen["failure"]["code"]=="INTEGRITY_MISMATCH"
    out={"proof_id":"PRF-NEXY-RUNTIME-001","status":"PASS","target_count":7,
         "target_ids":TARGETS,"success":{"execution_id":success["execution_id"],"result_id":success["id"],"evidence_id":success["evidence_id"],"chain":success["chain"]},
         "failure":{"execution_id":failure["execution_id"],"result_id":failure["id"],"evidence_id":failure["evidence_id"],"code":failure["failure"]["code"]},
         "freeze":{"execution_id":frozen["execution_id"],"result_id":frozen["id"],"evidence_id":frozen["evidence_id"],"code":frozen["failure"]["code"]}}
