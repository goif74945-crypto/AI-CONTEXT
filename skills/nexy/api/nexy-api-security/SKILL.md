---
name: nexy-api-security
description: Test API abuse: unauthorized access, invalid input, rate limit, CSRF where applicable, session, and RBAC.
---
...
## Objective
Test API abuse: unauthorized access, invalid input, rate limit, CSRF where applicable, session, and RBAC.
...
## Scope
### In Scope
unauthorized/forbidden requests, malformed input, rate/abuse controls, CSRF where applicable, session lifecycle, RBAC and secret/error leakage.
### Out of Scope
Unsafe or unauthorized attack activity, weakening controls, leaking credentials, or claiming secure from a single negative case.
...
## Workflow
Context → Authority/Threats → Auth/RBAC → Invalid Input → Rate/Replay/CSRF/Session → Error/Log Leakage → Regression → Evidence.

## Required Behavior
Use approved targets and fail-closed expectations; legitimate flows remain functional while abuse is blocked.
...
## Freeze Conditions
Test scope unauthorized/unsafe, secret encountered, destructive effect possible, or required security boundary/human gate unresolved.

## Validation
Use structural, source/spec, functional, security, architecture, integration, regression and evidence validation as applicable.

## Completion Criteria
Applicable API abuse paths are blocked as specified and legitimate behavior/regression remains intact with evidence.
