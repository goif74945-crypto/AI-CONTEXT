---
name: nexy-verify
description: Drive the final verification gate across requirement, scope, architecture, security, tests and evidence without converting partial proof into VERIFIED.
---
...
## Objective
Final verification controller: Requirement, Scope, Architecture, Security, Tests, Evidence. Drive the final verification gate across requirement, scope, architecture, security, tests and evidence without converting partial proof into VERIFIED.
...
## Scope
### In Scope
requirement coverage; scope compliance; architecture compatibility; security gates; applicable tests; current evidence; unresolved blockers and final verdict.
### Out of Scope
VERIFY from presence, stale evidence, inferred PASS, skipped gate, unsupported confidence or self-approval without proof.
...
## Workflow
Context → Authority/Scope → Requirement Trace → Architecture/Security → Tests → Evidence Freshness → Contradiction Check → Final Gate → Verdict.

## Required Behavior
Return VERIFIED only when every applicable required gate is proven; otherwise PARTIAL/FAIL/BLOCKED/UNKNOWN remains explicit.
...
## Freeze Conditions
Critical requirement/evidence conflict, stale HEAD, missing mandatory proof, failed security gate, unresolved authority conflict.
...
## Completion Criteria
All applicable verification gates are satisfied with current traceable evidence and no critical unresolved gap.
