---
name: nexy-change-control
description: Control modifications and record CHANGE_ID, request, affected files/modules, contract/security/test impact, rollback.
---
...
## Objective
Control modifications and record CHANGE_ID, request, affected files/modules, contract/security/test impact, rollback.
...
## Scope
### In Scope
authorized change request, affected surfaces, impact analysis, version/diff, rollback, approvals and evidence.
### Out of Scope
Untracked mutation, scope expansion, hidden side effects, missing rollback where required, or claiming a change without exact diff/evidence.
...
## Workflow
Context → Authority → CHANGE_ID/Request → Impact/Dependency → Allowed Surface → Change Record → Validation/Rollback → Evidence.

## Required Behavior
Every material modification is attributable, scoped, impact-assessed, versioned where applicable, and auditable.
...
## Freeze Conditions
Authority conflict, destructive ambiguity, missing required rollback/human gate, or stale target invalidates the change record.

## Validation
Use structural, source/spec, functional, security, architecture, integration, regression and evidence validation as applicable.

## Completion Criteria
Change-control record is complete, traceable and matches actual authorized state.
