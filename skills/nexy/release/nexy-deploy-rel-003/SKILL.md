---
name: nexy-deploy
description: Govern NEXY deployment execution only after all required pre-deploy gates are proven and applicable human/production controls are satisfied.
---
...
## Objective
Deployment only after tests, verification, release policy, and evidence. Govern NEXY deployment execution only after all required pre-deploy gates are proven and applicable human/production controls are satisfied.
...
## Scope
### In Scope
deployment target/environment; release-gate verdict; current tests/evidence; config/secrets references; rollout/canary; health/rollback; human approval when required.
### Out of Scope
Deploying from stale/unverified build, bypassing human gate, exposing secrets, skipping rollback/canary obligations, or claiming deployment without runtime evidence.
...
## Workflow
Context → Target Environment → Release Gate → Human Gate → Config/Secret References → Rollout Plan → Authorized Deployment → Health/Evidence → Rollback if Required.

## Required Behavior
Fail closed before mutation unless release requirements are satisfied; production mutation requires explicit applicable approval.
...
## Freeze Conditions
Production mutation lacks approval, rollback unavailable, release gate failed/stale, secret handling unsafe, destructive migration unresolved.
...
## Completion Criteria
Authorized deployment completes for the exact artifact/environment and post-deploy evidence confirms required health/state; otherwise not deployed/verified.
